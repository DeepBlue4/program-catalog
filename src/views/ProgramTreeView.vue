...
watch(
  ...
  (newId) => {
    if (newId) {
      ...
      if (
        selectedNode.value &&
        String(selectedNode.value.program_id) === String(newId)
      )
        return;
...
watch(
  ...
  (newData) => {
    ...
    if (newData && targetId) {
      if (
        !selectedNode.value ||
        String(selectedNode.value.program_id) !== String(targetId)
      ) {
...
<template>
  <div class="chart-wrapper m3-card elevated">
    ...
    <div class="chart-container">
      ...
      />
      <div v-else class="empty-state">No program data available.</div>
